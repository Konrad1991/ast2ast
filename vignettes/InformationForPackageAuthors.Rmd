...
The argument `a` keeps the vector type *Array<Double, Buffer<Double>>*, while `size` and `temp` are inferred as scalars (*etr::Integer* and *etr::Double*
respectively), since `length()` and single-element subsetting both produce scalars. A detailed explanation is deferred to the subsequent section.
...
SEXP bubbleSort(SEXP aSEXP) {
  etr::Array<etr::Double, etr::Buffer<etr::Double>> a = aSEXP;
  etr::Integer size;
  etr::Double temp;
  size = etr::length(a);
  for (const auto i : etr::colon(etr::Double(1.0), size)) {
    for (const auto j : etr::colon(etr::Double(1.0), (size - etr::Double(1.0)))) {
      if (etr::subset(a, j) > etr::subset(a, j + etr::Double(1.0))) {
        temp = etr::subset(a, j);
        etr::subset(a, j) = etr::subset(a, j + etr::Double(1.0));
        etr::subset(a, j + etr::Double(1.0)) = temp;
      }
    }
  }
  return(etr::Cast(a));
