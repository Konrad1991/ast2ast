...
### Fundamental Type in Expression Template Library R (ETR): Array Class

The core type in the expression template library R (ETR) is a class called *Array*.
...
The Array class incorporates three templates, namely *T*, *R*, and *Trait*.
...
The third template, *Trait*, plays a crucial role in endowing the class with identifiable properties as Array during the compile-time phase. 
...
Each of these classes contributes distinct functionalities and features to the *Array* class.
...
The class *Array* is thread safe in the sense that no memory is associated with functions or global variables. Moreover, almost no static methods are defined.
