#ifndef AT_TAGGED_BENCH_HPP
#define AT_TAGGED_BENCH_HPP

// Standalone copy of etr::at()'s first overload (SimplifyingSubsetting.hpp,
// the IsLBufferArray||IsBorrowArray branch) with the string_literal Expr NTTP
// and body-wrapping try/catch from development/issue_variable_names.md added.
// Kept as a separate function (at_tagged, not an overload of at) so it can
// coexist with the real at() from etr.hpp in the same translation unit for
// a like-for-like compile-time/binary-size comparison.

namespace etr {

template <string_literal Expr, typename ArrayType, typename... Args>
requires (IsLBufferArray<ArrayType> || IsBorrowArray<ArrayType>)
inline decltype(auto) at_tagged(ArrayType& arr, const Args&... args) {
  try {
    ...
    throw std::runtime_error(std::string("In '") + Expr.value.data() + "': " + e.what());
  }
}
