...
c     DUMMY is a placeholder C() argument for the NCV=0 (no vector to
c     transform) call to H12 below. Modern gfortran checks argument rank
c     consistency across all calls to the same external procedure even
c     without an explicit interface, so this has to be rank-1 to match
c     the real array (ZZ, or a column of A) passed at the other call
c     sites -- a bare scalar here is a compile error on current gfortran,
c     though it was harmless (and legal) in the original 1974 F77.
