...
inline VEC<T, VVMINUS<T, L, R>> operator-(const VEC<T, L> &a,
                                          const VEC<T, R> &b) {
...
// this only holds true for +, -, * and :. Because other functions require both
// (or all) arguments to compile successfully in R /2 and *2 does not work! Only
...
public:
  ...
  T operator[](const int i) const { return -l[i % l.size()]; }
