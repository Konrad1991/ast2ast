...
public:
  ...
  T get_deriv_left(const int i) const { return r[i % r.size()]; }

  T get_deriv_right(const int i) const { return l[i % l.size()]; }
...
inline VEC<T, VVTIMES<T, L, R>> operator*(const VEC<T, L> &a,
                                          const VEC<T, R> &b) {
